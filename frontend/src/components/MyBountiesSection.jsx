...
function MyBountyCard({ bounty, contract, address, onChanged }) {
  ...
  const isFreelancer =
    bounty.selectedFreelancer.toLowerCase() === address.toLowerCase();
  ...
  return (
    ...
    >
      <p>
        <strong>Bounty #{bounty.id}</strong> —{" "}
        {formatEther(bounty.escrowAmount)} ETH in escrow —{" "}
